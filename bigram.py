...
class InvertedIndexJob(MRJob):
    ...
    REV_BIGRAM_DICT = {val: key for key, val in BIGRAM_DICT.items()}
    ...
    def clean(self, text:str):
        ...
        for bigram, replacement in self.BIGRAM_DICT.items():
            text = re.sub(bigram, replacement, text)
        return text
    ...
    def mapper(self, key, value):
        ...
        for word in document:
            if word.startswith('<:') and word.endswith(':>'):
                yield self.REV_BIGRAM_DICT[word], doc_id
